...
struct bpf_map_def SEC("maps") sk_to_addr_map = {
...
int prog_connect(struct checmate_ctx *ctx)
{
	...
	rc = bpf_map_update_elem(&sk_to_addr_map, &sk, &addr_in, BPF_ANY);
	if (rc)
		return rc;
...
int prog_sk_free(struct checmate_ctx *ctx)
{
	...
	addr = bpf_map_lookup_elem(&sk_to_addr_map, &sk);
	if (!addr)
		return 0;
	...
	__sync_fetch_and_add(refcnt, -1);
	char fmt[] = "Connections2: %d\n";
	bpf_trace_printk(fmt, sizeof(fmt), *refcnt);

	bpf_map_delete_elem(&sk_to_addr_map, &sk);
